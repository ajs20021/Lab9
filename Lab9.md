...
\#this is a pretty good scale but the reverse scored measure does not
have high factor loading… (it is below .4)
...
Cronbach’s alpha of 0.83 demonstrates good reliability for this scale.
It has more than acceptable internal consistency between items in the
scale.
...
There’s only 1 factor here because the eigenvalue is above 1, its point
is on the slope not the the scree/flat part, and it is above the
parallel. The range of the loadings are 0.940 - 0.823. The item with the
highest loading is item 51 at 0.940. All factors are above .4 and very
close to 1, meaning they all highly influence the dependent variable.
...
\#For the affiliation (independence) scale, Cronbach’s alpha of .858 is
quite high so we can say the scale is internally consistent and its
...
![](Lab9_files/figure-gfm/unnamed-chunk-7-1.png)<!-- --> \#There’s only
1 factor here because the eigenvalue is above 1, its point is on the
slope not the the scree/flat part, and it exists above the parallel. The
range of the loadings are between .846 and .534. The item with the
highest loading is item 30 at .846. All factors are above .4. This means
they’re all pretty high predictors of variance in the dependent
variable. I would exercise caution with interpreting item 27 since it is
a bit close to .4. Of all the items, it would be the weakest predictor
